...
class User(db.Model, UserMixin):
    ...
    # Check if all mandatory fields are filled
        required_fields = [self.location, self.nin_number, self.nin_slip, self.bio, self.phone_number,
                      self.phone_number_two, self.street, self.city, self.state]
    ...
    @staticmethod
    def total_users_signed_up():
        return User.query.count()
    
    @staticmethod
    def total_active_users():
        # Define the threshold for considering a user as active
        active_threshold = datetime.utcnow() - timedelta(days=30)  # Assuming active if logged in within the last 30 days
        return User.query.filter(User.last_login_at >= active_threshold).count()
...
class Property(db.Model):
    ...
    property_subtype = db.Column(db.String, nullable=True)
    ...
    property_num_rooms = db.Column(db.Integer, nullable=True)
    property_num_baths = db.Column(db.Integer, nullable=True)
    property_num_toilets = db.Column(db.Integer, nullable=True)
    property_num_parlour = db.Column(db.Integer, nullable=True)
...
class Shared(db.Model):
    ...
    shared_subtype = db.Column(db.String, nullable=True)
    ...
    fully_furnished = db.Column(db.Boolean, nullable=True, default=False)
    partly_unfurnished = db.Column(db.Boolean, nullable=True, default=False)
    parking_space = db.Column(db.Boolean, nullable=True, default=False)
    laundry_room = db.Column(db.Boolean, nullable=True, default=False)
    shared_kitchen = db.Column(db.Boolean, nullable=True, default=False)
    shared_bathroom = db.Column(db.Boolean, nullable=True,default=False)
    noise_level = db.Column(db.Boolean, nullable=False, default=False)
    visitors = db.Column(db.Boolean, nullable=False, default=False)
    fitness_center = db.Column(db.Boolean, nullable=True,default =False)
    ...
    time_posted = db.Column(db.Time) 
    @staticmethod
    def total_shared_posted():
        return Shared.query.count()
    def save(self):
        db.session.add(self)
        db.session.commit()
...
class ShortLet(db.Model):
    ...
    shortlet_subtype = db.Column(db.String, nullable=True)
    ...
    shortlet_num_rooms = db.Column(db.Integer, nullable=True)
    ...
    shortlet_check_in = db.Column(db.Boolean, nullable=True, default=False)
    free_cancellation = db.Column(db.Boolean, nullable=True, default=False)
    fully_furnished = db.Column(db.Boolean, nullable=True, default=False)
    partly_unfurnished = db.Column(db.Boolean, nullable=True, default=False)
    parking_space = db.Column(db.Boolean, nullable=True, default=False)
    laundry_room = db.Column(db.Boolean, nullable=True, default=False)
    shared_kitchen = db.Column(db.Boolean, nullable=True, default=False)
    shared_bathroom = db.Column(db.Boolean, nullable=True,default=False)
    noise_level = db.Column(db.Boolean, nullable=True, default=False)
    visitors = db.Column(db.Boolean, nullable=True, default=False)
    fitness_center = db.Column(db.Boolean, nullable=True, default=False)
    internet_service = db.Column(db.Boolean, nullable=True, default=False)
    ...
    @staticmethod
    def total_shortlet_posted():
        return ShortLet.query.count()
    def save(self):
        db.session.add(self)
        db.session.commit()
...
class JVA(db.Model):
    ...
    jva_subtype = db.Column(db.String, nullable=True)
    ...
    time_posted = db.Column(db.Time) 
    @staticmethod
    def total_jva_posted():
        return JVA.query.count()
    def save(self):
        db.session.add(self)
        db.session.commit()
...
def create_shared():
    ...
    required_fields=['location','shared_property_ownership', 'shared_property_available', 'shared_negotiation', 'shared_price', 'shared_description']
    for field in required_fields:
            if not request.form.get(field):
                flash(f"Field '{field}' is missing. Please fill in all required fields.", 'error')
                return redirect(url_for('forsale'))
    if request.method == 'POST':
        # Extract form data
...
def create_shortlet():
    ...
    required_fields=['location','shortlet_property_ownership', 'shortlet_property_available', 'shortlet_negotiation', 'shortlet_price', 'shortlet_description']
    for field in required_fields:
            if not request.form.get(field):
                flash(f"Field '{field}' is missing. Please fill in all required fields.", 'error')
                return redirect(url_for('forrent'))
...
def create_jva():
    ...
    required_fields=['location','jva_property_ownership', 'jva_property_available', 'jva_negotiation', 'jva_price', 'jva_description']
    for field in required_fields:
            if not request.form.get(field):
                flash(f"Field '{field}' is missing. Please fill in all required fields.", 'error')
                return redirect(url_for('jv'))
    if request.method == 'POST':
        # Extract form data
...
def admin_dashboard():
    ...
    total_properties = Property.total_properties_posted()
    total_shortlet = ShortLet.total_shortlet_posted()
    total_shared =Shared.total_shared_posted()
    total_jva = JVA.total_jva_posted()
    return render_template('admin_dashboard.html', pending_properties=pending_properties, pending_shortlet = pending_shortlet, pending_shared = pending_shared, pending_jva = pending_jva, User=User, total_jva= total_jva, total_properties=total_properties, total_shared=total_shared, total_shortlet= total_shortlet)
...
def listing():
    user = current_user
    shared_instance, amenity = get_shared_instance_and_amenity()
...
def total_listing():
    ...
    return render_template('all_listing.html' ,shared_instance= shared_instance ,amenity= amenity ,  properties=properties, approved_properties=approved_properties, pending_properties=pending_properties, approved_shortlets = approved_shortlets, pending_shortlets = pending_shortlets, shortlet = shortlet, shared = shared, approved_shared = approved_shared, pending_shared = pending_shared, jva = jva, approved_jva= approved_jva, pending_jva = pending_jva, user=user)




def get_shared_instance_and_amenity():
    ...
    return shared_instance, amenity
...
def listinguser():
    ...
    # Assuming similar models exist for shared, shortlet, and jva properties
    approved_shared = Shared.query.filter_by(approved=True).all()
    pending_shared = Shared.query.filter_by(approved=False).all()
...
# Define route for Property listings
@app.route('/property-listing')
def property_listing():
    user = current_user
    properties = Property.query.all()
    approved_properties = Property.query.filter_by(approved=True).all()
    pending_properties = Property.query.filter_by(approved=False).all()
    return render_template('property_listing.html', properties=properties, approved_properties=approved_properties, pending_properties=pending_properties, user=user)

# Define route for ShortLet listings
@app.route('/shortlet-listing')
def shortlet_listing():
    user = current_user
    shortlets = ShortLet.query.all()
    approved_shortlets = ShortLet.query.filter_by(approved=True).all()
    pending_shortlets = ShortLet.query.filter_by(approved=False).all()
    return render_template('shortlet_listing.html', shortlets=shortlets, approved_shortlets=approved_shortlets, pending_shortlets=pending_shortlets, user=user)

# Define route for Shared listings
@app.route('/shared-listing')
def shared_listing():
    user = current_user
    shared_listings = Shared.query.all()
    approved_shared = Shared.query.filter_by(approved=True).all()
    pending_shared = Shared.query.filter_by(approved=False).all()
    shared_instance = Shared.query.first()
    amenity = shared_instance.amenities if shared_instance else None
    return render_template('shared_listing.html', shared_listings=shared_listings, approved_shared=approved_shared, pending_shared=pending_shared, shared_instance=shared_instance, amenity=amenity, user=user)

# Define route for JVA listings
@app.route('/jva-listing')
def jva_listing():
    user = current_user
    jvas = JVA.query.all()
    approved_jvas = JVA.query.filter_by(approved=True).all()
    pending_jvas = JVA.query.filter_by(approved=False).all()
    return render_template('jva_listing.html', jvas=jvas, approved_jvas=approved_jvas, pending_jvas=pending_jvas, user=user)
...
def property_details(property_id):
    ...
    # if not current_user.is_authenticated:
    #     flash('Please log in to add items to your Bookmark.', 'info')
    #     return redirect(url_for('login'))
    ...
    if request.method == 'POST':
        ...
        if 'add_to_cart' in request.form:
            ...
            try:
                db.session.commit()
                flash('Property added to your Bookmark', 'success')
            except Exception as e:
                db.session.rollback()
                flash('An error occurred while adding the property to your Bookmark. Please try again.', 'error')
                app.logger.error(f"Error adding property to Bookmark: {e}")
...
def shortlet_details(shortlet_id):
    ...
    if request.method == 'POST':
        ...
        if 'add_to_cart' in request.form:
            ...
            try:
                db.session.commit()
                flash('Shortlet added to your Bookmark', 'success')
            except Exception as e:
                db.session.rollback()
                flash('An error occurred while adding the property to your Bookmark. Please try again.', 'error')
                app.logger.error(f"Error adding property to cart: {e}")
...
def shared_details(shared_id):
    ...
    if request.method == 'POST':
        ...
        if 'add_to_cart' in request.form:
            ...
            try:
                db.session.commit()
                flash('Shared property added to your Bookmark', 'success')
            except Exception as e:
                db.session.rollback()
                flash('An error occurred while adding the property to your Bookmark. Please try again.', 'error')
                app.logger.error(f"Error adding property to cart: {e}")
...
def jva_details(jva_id):
    ...
    if request.method == 'POST':
        ...
        if 'add_to_cart' in request.form:
            ...
            try:
                db.session.commit()
                flash('JVA property added to your Bookmark', 'success')
            except Exception as e:
                db.session.rollback()
                flash('An error occurred while adding the property to your cart. Please try again.', 'error')
                app.logger.error(f"Error adding property to cart: {e}")
...
def dashboard():
    ...
    print("shared_instance:", shared_instance, "amenity:", amenity, "properties:", properties, "approved_properties:", approved_properties, "pending_properties:", pending_properties, "approved_shortlets:", approved_shortlets, "pending_shortlets:", pending_shortlets, "shortlet:", shortlet, "shared:", shared, "approved_shared:", approved_shared, "pending_shared:", pending_shared, "jva:", jva, "approved_jva:", approved_jva, "pending_jva:", pending_jva, "user:", user)
   
    
    return render_template('dashboard.html', notifications=user_notifications ,shared_instance= shared_instance ,amenity= amenity ,  properties=properties, approved_properties=approved_properties, pending_properties=pending_properties, approved_shortlets = approved_shortlets, pending_shortlets = pending_shortlets, shortlet = shortlet, shared = shared, approved_shared = approved_shared, pending_shared = pending_shared, jva = jva, approved_jva= approved_jva, pending_jva = pending_jva, user=user)
    ...
    if request.method == 'POST':
        try:
            ...
            if user:
                ...
                user.location = request.form.get('house_number', '')
                user.nin_number = request.form.get('nin_number', '')
                user.phone_number_two = request.form.get('alitnumber', '')
...
@app.route('/admin/property/<int:property_id>')
def admin_property_details(property_id):
    # Fetch property details from the database based on property_id
    property = Property.query.get_or_404(property_id)
    # Render template with property details
    return render_template('admin_property_details.html', property=property)
    
@app.route('/admin/shared_property/<int:shared_id>')
def admin_shared_details(shared_id):
    # Fetch shared property details from the database based on shared_id
    shared_property = Shared.query.get_or_404(shared_id)
    # Render template with shared property details
    return render_template('admin_shared_details.html', shared=shared_property)


@app.route('/admin/shortlet_property/<int:shortlet_id>')
def admin_shortlet_property_details(shortlet_id):
    # Fetch shortlet property details from the database based on property_id
    shortlet_property = ShortLet.query.get_or_404(shortlet_id)
    # Render template with shortlet property details
    return render_template('admin_shortlet_details.html', shortlet=shortlet_property)


@app.route('/admin/jva_property/<int:jva_id>')
def admin_jva_property_details(jva_id):
    # Fetch JVA property details from the database based on property_id
    jva_property = JVA.query.get_or_404(jva_id)
    # Render template with JVA property details
    return render_template('admin_jva_details.html', jva=jva_property)

@app.route('/view_agent')
def view_agent():
    return render_template('view-agent-property-profile.html')
